...
impl Token {
    pub fn unwrap_expression(input: Token) -> Vec<Token> {
        match input {
            Token::Expression(exp, _, _) => {
                let output: Vec<Token> = exp;
                output
            }
            x => {
                println!("Must input an Expression not {}", x);
                std::process::exit(1)
            }
        }
    }
...
fn scan(input: String, line: u16) -> Vec<Token> {
    ...
    output
        .into_iter()
        .filter(|x| match x {
            Token::Invalid(x, _, _) => !x.is_empty(),
            Token::Identifier(x, _, _) => !x.is_empty(),
            _ => true,
        })
        .collect()
}
fn evaluate(input: String, col: u16, line: u16) -> Token {
    match input.len() {
        ...
        _ => match input.chars().next().unwrap_or_else(|| {
            println!("invalid Token {} at {},{}", input, col, line);
            std::process::exit(1)
        }) {
            '1'..='9' => Token::Number(input, col, line),
            'a'..='z' => Token::Identifier(input, col, line),
...
fn reverse_vec<T>(input: &Vec<T>) -> Vec<T>
where
    T: Clone,
{
...
pub fn tokenizer(input: String) -> Vec<Vec<Token>> {
    ...
    for (line, l) in lines.into_iter().enumerate() {
        let mut inp: Vec<Token> = scan(l, line as u16);
